...
public class AlarmUtilities {
	...
	public static void addAlarmDialog() {
		...
		CButton cancel = new CButton("Cancel", "cancel and go back", 'C', KeyStroke.getKeyStroke(KeyEvent.VK_ESCAPE, 0),
				null);
		JPanel bPanel = new JPanel(new FlowCustomLayout(FlowLayout.RIGHT));
		JPanel mainPanel = new JPanel(new GridLayout(1, 2)), left = new JPanel(new FlowCustomLayout(FlowLayout.LEFT)),
				right = new JPanel();
		...
		mPanel.setBorder(
				BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(EtchedBorder.LOWERED), "Message"));
		...
		add.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				AlarmUtilities.getInstance().addToList(new Alarm(calender.getDate(), message.getText(),
						high.isSelected() ? Alarm.HIGH : mid.isSelected() ? Alarm.MED : Alarm.LOW));
	...
	public static void showEditDialog(final Alarm e) {
		...
		final JRadioButton low = new JRadioButton("Low"), mid = new JRadioButton("Mid"),
		...
		JPanel north = new JPanel(new FlowLayout()), cent = new JPanel(new GridLayout(1, 1)),
				south = new JPanel(new GridLayout(1, 1)), bPanel = new JPanel(new FlowCustomLayout(FlowLayout.RIGHT));
