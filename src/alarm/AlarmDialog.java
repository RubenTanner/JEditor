...
public class AlarmDialog {
	...
	public JPanel getMenuPanel() {
		...
		edit.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (AlarmTable.getInstance().getRealTable().getSelectedRowCount() == 0) {
					JOptionPane.showMessageDialog(AlarmDialog.getInstance().getDialog(),
							"There is no row selected to edit.", "Error", JOptionPane.ERROR_MESSAGE);
					return;
				}

				if (AlarmTable.getInstance().getRealTable().getSelectedRowCount() > 1) {
					JOptionPane.showMessageDialog(AlarmDialog.getInstance().getDialog(),
							"Please select a single row to edit.", "Error", JOptionPane.ERROR_MESSAGE);
					return;
				}

				AlarmUtilities.showEditDialog(AlarmUtilities.getInstance().getList()
						.get(AlarmTable.getInstance().getRealTable().getSelectedRow()));
		...
		remove.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (AlarmTable.getInstance().getRealTable().getSelectedRowCount() == 0) {
					JOptionPane.showMessageDialog(AlarmDialog.getInstance().getDialog(),
							"There is no row selected to remove.", "Error", JOptionPane.ERROR_MESSAGE);
					return;
				}

				if (AlarmTable.getInstance().getRealTable().getSelectedRowCount() > 1) {
					JOptionPane.showMessageDialog(AlarmDialog.getInstance().getDialog(),
							"Please select a single row to remove.", "Error", JOptionPane.ERROR_MESSAGE);
					return;
				}

				int result = JOptionPane.showConfirmDialog(AlarmDialog.getInstance().getDialog(),
						"Are you sure you want to remove the selected alarm?", "Confirm",
	...
	public JPanel getMainPanel() {
		...
		panel.setBorder(
				BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(EtchedBorder.LOWERED), "Alarms"));
		return panel;
	}
