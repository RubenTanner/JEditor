...
public class TablePopupMenu extends JPopupMenu {
	...
	public void init() {
		edit = new CMenuItem("Edit", "edit the alarm", 'E',
				KeyStroke.getKeyStroke(KeyEvent.VK_E, InputEvent.CTRL_DOWN_MASK));
		remove = new CMenuItem("Remove", "remove the selected alarm", 'R',
				KeyStroke.getKeyStroke(KeyEvent.VK_DELETE, 0));
		removeAll = new CMenuItem("Remove all", "remove all the alarms", 'A',
				KeyStroke.getKeyStroke(KeyEvent.VK_DELETE, InputEvent.SHIFT_DOWN_MASK));
		selectAll = new CMenuItem("Select all", "select all the alarms", 'S',
				KeyStroke.getKeyStroke(KeyEvent.VK_A, InputEvent.CTRL_DOWN_MASK));
		clearSelection = new CMenuItem("Clear selection", "clear the selection form the tabkle", 'C',
		...
		removeAll.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				int result = JOptionPane.showConfirmDialog(AlarmDialog.getInstance().getDialog(),
						"Are you sure you want to delete all the alarms?", "Confirm", JOptionPane.YES_NO_CANCEL_OPTION);
